...
class SLat2Render(StandardDatasetBase):
    ...
    def __init__(self, roots: str, image_size: int, latent_model: str, min_aesthetic_score: float=5.0, max_num_voxels: int=32768):
    ...
    def collate_fn(batch):
        ...
        pack['latents'] = SparseTensor(coords=coords, feats=feats)
...
class Slat2RenderGeo(SLat2Render):
    ...
    def __init__(self, roots: str, image_size: int, latent_model: str, min_aesthetic_score: float=5.0, max_num_voxels: int=32768):
        super().__init__(roots, image_size, latent_model, min_aesthetic_score, max_num_voxels)

    def _get_geo(self, root, instance):
        (verts, face) = utils3d.io.read_ply(os.path.join(root, 'renders', instance, 'mesh.ply'))
        mesh = {'vertices': torch.from_numpy(verts), 'faces': torch.from_numpy(face)}
        return {'mesh': mesh}
