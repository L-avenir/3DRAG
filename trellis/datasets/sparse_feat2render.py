...
class SparseFeat2Render(StandardDatasetBase):
    ...
    def __init__(self, roots: str, image_size: int, model: str='dinov2_vitl14_reg', resolution: int=64, min_aesthetic_score: float=5.0, max_num_voxels: int=32768):
    ...
    def _get_image(self, root, instance):
        ...
        return {'image': image, 'alpha': alpha, 'extrinsics': extrinsics, 'intrinsics': intrinsics}
    ...
    def _get_feat(self, root, instance):
        ...
        if self.resolution != DATA_RESOLUTION:
            ...
            feats = torch.scatter_reduce(torch.zeros(coords.shape[0], feats.shape[1], device=feats.device), dim=0, index=idx.unsqueeze(-1).expand(-1, feats.shape[1]), src=feats, reduce='mean')
    ...
    def collate_fn(batch):
        ...
        pack['feats'] = SparseTensor(coords=coords, feats=feats)
