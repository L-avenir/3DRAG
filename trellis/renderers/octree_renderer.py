...
def render(viewpoint_camera, octree: DfsOctree, pipe, bg_color: torch.Tensor, scaling_modifier=1.0, used_rank=None, colors_overwrite=None, aux=None, halton_sampler=None):
    ...
    if octree.primitive == 'voxel':
        ...
        (rgb, depth, alpha, distloss) = renderer(positions=positions, densities=densities, shs=shs, colors_precomp=colors_precomp, depths=depths, aabb=octree.aabb, aux=aux)
        ret['rgb'] = rgb
        ret['depth'] = depth
        ret['alpha'] = alpha
        ...
        (rgb, depth, alpha) = renderer(positions=positions, opacities=opacities, shs=shs, colors_precomp=colors_precomp, depths=depths, aabb=octree.aabb, aux=aux)
        ret['rgb'] = rgb
        ret['depth'] = depth
        ret['alpha'] = alpha
    elif octree.primitive == 'trivec':
        ...
        (rgb, depth, alpha, percent_depth) = renderer(positions=positions, trivecs=trivecs, densities=densities, shs=shs, colors_precomp=colors_precomp, colors_overwrite=colors_overwrite, depths=depths, aabb=octree.aabb, aux=aux, halton_sampler=halton_sampler)
        ...
        (rgb, depth, alpha) = renderer(positions=positions, decoupolys_V=decoupolys_V, decoupolys_g=decoupolys_g, densities=densities, shs=shs, colors_precomp=colors_precomp, depths=depths, aabb=octree.aabb, aux=aux)
        ret['rgb'] = rgb
        ret['depth'] = depth
        ret['alpha'] = alpha
    return ret
...
class OctreeRenderer:
    ...
    def __init__(self, rendering_options={}) -> None:
        try:
            ...
            print('\x1b[93m[WARNING] diffoctreerast is not installed. The renderer will be disabled.\x1b[0m')
            self.unsupported = True
        else:
            self.unsupported = False
        self.pipe = edict({'with_distloss': False, 'with_aux': False, 'scale_modifier': 1.0, 'used_rank': None, 'jitter': False, 'debug': False})
        self.rendering_options = edict({'resolution': None, 'near': None, 'far': None, 'ssaa': 1, 'bg_color': 'random'})
        self.halton_sampler = qmc.Halton(2, scramble=False)
        self.rendering_options.update(rendering_options)
        self.bg_color = None

    def render(self, octree: DfsOctree, extrinsics: torch.Tensor, intrinsics: torch.Tensor, colors_overwrite: torch.Tensor=None) -> edict:
        ...
        if self.unsupported:
            ...
            return {'color': torch.tensor(image, dtype=torch.float32).permute(2, 0, 1) / 255}
        ...
        if self.pipe['with_aux']:
            aux = {'grad_color2': torch.zeros((octree.num_leaf_nodes, 3), dtype=torch.float32, requires_grad=True, device='cuda') + 0, 'contributions': torch.zeros((octree.num_leaf_nodes, 1), dtype=torch.float32, requires_grad=True, device='cuda') + 0}
            for k in aux.keys():
        ...
        camera_dict = edict({'image_height': resolution * ssaa, 'image_width': resolution * ssaa, 'FoVx': fovx, 'FoVy': fovy, 'znear': near, 'zfar': far, 'world_view_transform': view.T.contiguous(), 'projection_matrix': perspective.T.contiguous(), 'full_proj_transform': (perspective @ view).T.contiguous(), 'camera_center': camera})
        ...
        ret = edict({'color': render_ret.rgb, 'depth': render_ret.depth, 'alpha': render_ret.alpha})
