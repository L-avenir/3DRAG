...
def render(viewpoint_camera, pc: Gaussian, pipe, bg_color: torch.Tensor, scaling_modifier=1.0, override_color=None):
    ...
    return edict({'render': rendered_image, 'viewspace_points': screenspace_points, 'visibility_filter': radii > 0, 'radii': radii})

class GaussianRenderer:

    def __init__(self, rendering_options={}) -> None:
        self.pipe = edict({'kernel_size': 0.1, 'convert_SHs_python': False, 'compute_cov3D_python': False, 'scale_modifier': 1.0, 'debug': False})
        self.rendering_options = edict({'resolution': None, 'near': None, 'far': None, 'ssaa': 1, 'bg_color': 'random'})
        self.rendering_options.update(rendering_options)
        self.bg_color = None

    def render(self, gausssian: Gaussian, extrinsics: torch.Tensor, intrinsics: torch.Tensor, colors_overwrite: torch.Tensor=None) -> edict:
