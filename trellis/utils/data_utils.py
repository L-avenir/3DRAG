...
def recursive_to_device(data: Any, device: torch.device, non_blocking: bool=False) -> Any:
    if hasattr(data, 'to'):
...
def load_balanced_group_indices(load: List[int], num_groups: int, equal_size: bool=False) -> List[List[int]]:
    if equal_size:
...
class ResumableSampler(Sampler):
    ...
    def __init__(self, dataset: Dataset, shuffle: bool=True, seed: int=0, drop_last: bool=False) -> None:
        ...
        if self.drop_last and len(self.dataset) % self.world_size != 0:
            self.num_samples = math.ceil((len(self.dataset) - self.world_size) / self.world_size)
        else:
...
class BalancedResumableSampler(ResumableSampler):
    ...
    def __init__(self, dataset: Dataset, shuffle: bool=True, seed: int=0, drop_last: bool=False, batch_size: int=1) -> None:
