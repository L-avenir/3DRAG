...
class SLatRadianceFieldDecoder(SparseTransformerBase):
    ...
    def _calc_layout(self) -> None:
        self.layout = {'trivec': {'shape': (self.rep_config['rank'], 3, self.rep_config['dim']), 'size': self.rep_config['rank'] * 3 * self.rep_config['dim']}, 'density': {'shape': (self.rep_config['rank'],), 'size': self.rep_config['rank']}, 'features_dc': {'shape': (self.rep_config['rank'], 1, 3), 'size': self.rep_config['rank'] * 3}}
        start = 0
    ...
    def to_representation(self, x: sp.SparseTensor) -> List[Strivec]:
        ...
        for i in range(x.shape[0]):
            representation = Strivec(sh_degree=0, resolution=self.resolution, aabb=[-0.5, -0.5, -0.5, 1, 1, 1], rank=self.rep_config['rank'], dim=self.rep_config['dim'], device='cuda')
