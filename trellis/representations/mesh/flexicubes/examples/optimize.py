...
if __name__ == '__main__':
    ...
    for it in range(FLAGS.iter):
        ...
        grid_verts = x_nx3 + (2 - 1e-08) / (FLAGS.voxel_grid_res * 2) * torch.tanh(deform)
        (vertices, faces, L_dev) = fc(grid_verts, sdf, cube_fx8, FLAGS.voxel_grid_res, beta_fx12=weight[:, :12], alpha_fx8=weight[:, 12:20], gamma_f=weight[:, 20], training=True)
        flexicubes_mesh = Mesh(vertices, faces)
        buffers = render.render_mesh_paper(flexicubes_mesh, mv, mvp, FLAGS.train_res)
        mask_loss = (buffers['mask'] - target['mask']).abs().mean()
        depth_loss = ((((buffers['depth'] - target['depth']) * target['mask']) ** 2).sum(-1) + 1e-08).sqrt().mean() * 10
        ...
        if FLAGS.sdf_loss:
            ...
            total_loss += torch.nn.functional.mse_loss(pred_sdf, gt_sdf) * 2000.0
        ...
        optimizer.step()
        scheduler.step()
        if it % FLAGS.save_interval == 0 or it == FLAGS.iter - 1:
            with torch.no_grad():
                (vertices, faces, L_dev) = fc(grid_verts, sdf, cube_fx8, FLAGS.voxel_grid_res, beta_fx12=weight[:, :12], alpha_fx8=weight[:, 12:20], gamma_f=weight[:, 20], training=False)
                flexicubes_mesh = Mesh(vertices, faces)
