...
def get_random_camera_batch(batch_size, fovy=np.deg2rad(45), iter_res=[512, 512], cam_near_far=[0.1, 1000.0], cam_radius=3.0, device='cuda', use_kaolin=True):
    if use_kaolin:
        camera_pos = torch.stack(kal.ops.coords.spherical2cartesian(*kal.ops.random.sample_spherical_coords((batch_size,), azimuth_low=0.0, azimuth_high=math.pi * 2, elevation_low=-math.pi / 2.0, elevation_high=math.pi / 2.0, device='cuda'), cam_radius), dim=-1)
        return kal.render.camera.Camera.from_args(eye=camera_pos + torch.rand((batch_size, 1), device='cuda') * 0.5 - 0.25, at=torch.zeros(batch_size, 3), up=torch.tensor([[0.0, 1.0, 0.0]]), fov=fovy, near=cam_near_far[0], far=cam_near_far[1], height=iter_res[0], width=iter_res[1], device='cuda')
    else:
...
def get_rotate_camera(itr, fovy=np.deg2rad(45), iter_res=[512, 512], cam_near_far=[0.1, 1000.0], cam_radius=3.0, device='cuda', use_kaolin=True):
    if use_kaolin:
        ...
        return kal.render.camera.Camera.from_args(eye=camera_pos, at=torch.zeros(3), up=torch.tensor([0.0, 1.0, 0.0]), fov=fovy, near=cam_near_far[0], far=cam_near_far[1], height=iter_res[0], width=iter_res[1], device='cuda')
    else:
        proj_mtx = util.perspective(fovy, iter_res[1] / iter_res[0], cam_near_far[0], cam_near_far[1])
        ang = itr / 10 * np.pi * 2
...
def render_mesh(mesh, camera, iter_res, return_types=['mask', 'depth'], white_bg=False, wireframe_thickness=0.4):
    ...
    for type in return_types:
        if type == 'mask':
            ...
            img = torch.logical_or(torch.logical_or(rast[..., 0] < wireframe_thickness, rast[..., 1] < wireframe_thickness), rast[..., 0] + rast[..., 1] > 1.0 - wireframe_thickness).unsqueeze(-1)
        elif type == 'normals':
            img = dr.interpolate(mesh.face_normals.reshape(len(mesh), -1, 3), rast, torch.arange(mesh.faces.shape[0] * 3, device='cuda', dtype=torch.int).reshape(-1, 3))[0]
        if white_bg:
...
def render_mesh_paper(mesh, mv, mvp, iter_res, return_types=['mask', 'depth'], white_bg=False):
    ...
    (rast, db) = dr.rasterize(dr.RasterizeGLContext(), v_pos_clip, mesh.faces.int(), iter_res)
    out_dict = {}
    for type in return_types:
        if type == 'mask':
...
class SplitVisualizer:
    ...
    def render(self, camera):
        lh_outputs = render_mesh(self.lh_mesh, camera, (self.height, self.width), return_types=['normals', 'wireframe'], wireframe_thickness=self.wireframe_thickness)
        rh_outputs = render_mesh(self.rh_mesh, camera, (self.height, self.width), return_types=['normals', 'wireframe'], wireframe_thickness=self.wireframe_thickness)
        outputs = {k: torch.cat([lh_outputs[k][0].permute(1, 0, 2), rh_outputs[k][0].permute(1, 0, 2)], dim=0).permute(1, 0, 2) for k in ['normals', 'wireframe']}
        return {'img': (outputs['wireframe'] * ((outputs['normals'] + 1.0) / 2.0) * 255).to(torch.uint8), 'normals': outputs['normals']}

    def show(self, init_camera):
        visualizer = kal.visualize.IpyTurntableVisualizer(self.height, self.width * 2, copy.deepcopy(init_camera), self.render, max_fps=24, world_up_axis=1)

        def slider_callback(new_wireframe_thickness):
            with visualizer.out:
                self.wireframe_thickness = new_wireframe_thickness
                visualizer.render_update()
        wireframe_thickness_slider = FloatLogSlider(value=self.wireframe_thickness, base=10, min=-3, max=-0.4, step=0.1, description='wireframe_thickness', continuous_update=True, readout=True, readout_format='.3f')
        interactive_slider = interactive(slider_callback, new_wireframe_thickness=wireframe_thickness_slider)
...
class TimelineVisualizer:
    ...
    def render(self, camera):
        outputs = render_mesh(self.meshes[self.idx], camera, (self.height, self.width), return_types=['normals', 'wireframe'], wireframe_thickness=self.wireframe_thickness)
        return {'img': (outputs['wireframe'] * ((outputs['normals'] + 1.0) / 2.0) * 255).to(torch.uint8)[0], 'normals': outputs['normals'][0]}

    def show(self, init_camera):
        visualizer = kal.visualize.IpyTurntableVisualizer(self.height, self.width, copy.deepcopy(init_camera), self.render, max_fps=24, world_up_axis=1)
        ...
        def slider_callback(new_wireframe_thickness, new_idx):
            with visualizer.out:
                ...
                visualizer.render_update()
        wireframe_thickness_slider = FloatLogSlider(value=self.wireframe_thickness, base=10, min=-3, max=-0.4, step=0.1, description='wireframe_thickness', continuous_update=True, readout=True, readout_format='.3f')
        idx_slider = IntSlider(value=self.idx, min=0, max=len(self.meshes) - 1, description='idx', continuous_update=True, readout=True)
        interactive_slider = interactive(slider_callback, new_wireframe_thickness=wireframe_thickness_slider, new_idx=idx_slider)
