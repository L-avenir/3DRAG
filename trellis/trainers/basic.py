...
class BasicTrainer(Trainer):
    ...
    def init_models_and_more(self, **kwargs):
        if self.world_size > 1:
            self.training_models = {name: DDP(model, device_ids=[self.local_rank], output_device=self.local_rank, bucket_cap_mb=128, find_unused_parameters=False) for (name, model) in self.models.items()}
        else:
        ...
        if self.elastic_controller_config is not None:
            assert any([isinstance(model, (elastic_utils.ElasticModule, elastic_utils.ElasticModuleMixin)) for model in self.models.values()]), 'No elastic module found in models, please inherit from ElasticModule or ElasticModuleMixin'
    ...
    def _master_params_to_state_dicts(self, master_params):
        ...
        master_params_names = sum([[(name, n) for (n, p) in model.named_parameters() if p.requires_grad] for (name, model) in self.models.items()], [])
    ...
    def save(self):
        ...
        misc_ckpt = {'optimizer': self.optimizer.state_dict(), 'step': self.step, 'data_sampler': self.data_sampler.state_dict()}
        if self.fp16_mode == 'amp':
    ...
    def run_step(self, data_list):
        ...
        if self.fp16_mode == 'amp':
            ...
            if not any((not p.grad.isfinite().all() for p in self.model_params)):
                ...
                print('\n\x1b[93mWarning: NaN detected in gradients. Skipping update.\x1b[0m')
        if self.lr_scheduler_config is not None:
