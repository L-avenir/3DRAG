...
class SLatVaeMeshDecoderTrainer(BasicTrainer):
    ...
    def __init__(self, *args, depth_loss_type: str='l1', lambda_depth: int=1, lambda_ssim: float=0.2, lambda_lpips: float=0.2, lambda_tsdf: float=0.01, lambda_color: float=0.1, **kwargs):
    ...
    def _render_batch(self, reps: List[MeshExtractResult], extrinsics: torch.Tensor, intrinsics: torch.Tensor, return_types=['mask', 'normal', 'depth']) -> Dict[str, torch.Tensor]:
    ...
    def _flip_normal(self, normal: torch.Tensor, extrinsics: torch.Tensor, intrinsics: torch.Tensor) -> torch.Tensor:
        ...
        R[:, 3, 3] = 1.0
        view_dir = utils3d.torch.unproject_cv(utils3d.torch.image_uv(*normal.shape[-2:], device=self.device).reshape(1, -1, 2), torch.ones(*normal.shape[-2:], device=self.device).reshape(1, -1), R, intrinsics).reshape(-1, *normal.shape[-2:], 3).permute(0, 3, 1, 2)
    ...
    def _perceptual_loss(self, gt: torch.Tensor, pred: torch.Tensor, name: str) -> Dict[str, torch.Tensor]:
        ...
        terms = {f'{name}_loss': l1_loss(gt, pred), f'{name}_loss_ssim': 1 - ssim(gt, pred), f'{name}_loss_lpips': lpips(gt, pred)}
    ...
    def geometry_losses(self, reps: List[MeshExtractResult], mesh: List[Dict], normal_map: torch.Tensor, extrinsics: torch.Tensor, intrinsics: torch.Tensor):
        with torch.no_grad():
            gt_meshes = []
    ...
    def training_losses(self, latents: SparseTensor, image: torch.Tensor, alpha: torch.Tensor, mesh: List[Dict], extrinsics: torch.Tensor, intrinsics: torch.Tensor, normal_map: torch.Tensor=None) -> Tuple[Dict, Dict]:
    ...
    def run_snapshot(self, num_samples: int, batch_size: int, verbose: bool=False) -> Dict:
        ...
        gt_render_results = self._render_batch([MeshExtractResult(vertices=mesh['vertices'].to(self.device), faces=mesh['faces'].to(self.device)) for mesh in gt_meshes], exts, ints, return_types=['normal'])
        ...
        multiview_normals = torch.cat([torch.cat(multiview_normals[:2], dim=-2), torch.cat(multiview_normals[2:], dim=-2)], dim=-1)
        ...
        if 'normal_map' in return_types:
            multiview_normal_maps = torch.cat([torch.cat(multiview_normal_maps[:2], dim=-2), torch.cat(multiview_normal_maps[2:], dim=-2)], dim=-1)
